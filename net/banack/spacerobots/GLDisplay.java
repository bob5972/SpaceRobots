...
public class GLDisplay implements GLEventListener, Display
{
	...
	private void renderShips(GLAutoDrawable drawable, DisplayFrame displayFrame)
	{
		GL gl = drawable.getGL();
	...
	private void renderTeamText(GLAutoDrawable drawable, DisplayFrame displayFrame)
	{
		...
		{
			boldRenderer.draw("" + displayFrame.teams.get(i).name, (drawable.getWidth() / displayFrame.teams.size())
					* i, drawable.getHeight() - 12);
		}
		...
		{
			plainRenderer.draw("   " + displayFrame.fleets.get(i).name + " (" + displayFrame.fleets.get(i).aiName + ":"
					+ displayFrame.fleets.get(i).aiVersion + ") -- " + displayFrame.fleets.get(i).aiAuthor,
					displayFrame.fleets.get(i).team.index * (drawable.getWidth() / displayFrame.teams.size()), drawable
							.getHeight()
							- 12 * (2 * displayFrame.fleets.get(i).indexInTeam + 2));
			plainRenderer.draw("       Credits: " + displayFrame.fleets.get(i).credits,
					displayFrame.fleets.get(i).team.index * (drawable.getWidth() / displayFrame.teams.size()), drawable
							.getHeight()
							- 12 * (2 * displayFrame.fleets.get(i).indexInTeam + 3));
		}
		...
		for (int i = 0; i < displayFrame.fleets.size(); i++)
		{
			gl.glColor4f(displayFrame.fleets.get(i).red, displayFrame.fleets.get(i).green,
					displayFrame.fleets.get(i).blue, 0.8f);
	...
	private void renderFrameStatsText(GLAutoDrawable drawable, DisplayFrame displayFrame)
	{
		...
		plainRenderer.draw("Ship Count: " + displayFrame.ships.size(), 0, 0);
		plainRenderer.endRendering();
	}
	...
	private void renderShip(GLAutoDrawable drawable, DisplayShip ship, double xOffset, double yOffset)
	{
		...
		gl.glVertex2f((float) (ship.scanner.getCenter().getX() + Math.cos(ship.scanner.getAngleStart())
				* ship.scanner.getRadius() + xOffset), (float) (ship.scanner.getCenter().getY()
				+ Math.sin(ship.scanner.getAngleStart()) * ship.scanner.getRadius() + yOffset));
		gl.glVertex2f((float) (ship.scanner.getCenter().getX() + xOffset),
				(float) (ship.scanner.getCenter().getY() + yOffset));
		gl.glVertex2f((float) (ship.scanner.getCenter().getX() + Math.cos(ship.scanner.getAngleEnd())
				* ship.scanner.getRadius() + xOffset), (float) (ship.scanner.getCenter().getY()
				+ Math.sin(ship.scanner.getAngleEnd()) * ship.scanner.getRadius() + yOffset));
		gl.glEnd();
	}
