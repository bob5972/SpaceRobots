...
public class Mob extends AIFleet
{
	        ...
	        double w, double h)
	{
		...
		destination = new DPoint(random.nextDouble() * 300, random.nextDouble() * 300);
		timeOutTick = 500;
	}
	...
	public Iterator<ShipAction> runTick()
	{
		...
		myCruiser.setHeading(shortestAngleToPoint(myCruiser.getPosition(), destination));
		center = myCruiser.getPosition();
		
		if (((Math.abs(myCruiser.getPosition().getX() - destination.getX()) < 10) && (Math.abs(myCruiser.getPosition()
		        .getY()
		        - destination.getY()) < 10))
		        || tick > timeOutTick) {
			
			destination = new DPoint(random.nextDouble() * battleHeight, random.nextDouble() * battleHeight);
		...
		while (iter.hasNext()) {
			...
			if (cur.getTypeID() == FIGHTER_ID || cur.getTypeID() == DESTROYER_ID) {
				if (myContacts.shipSize() == 0) {
					DPoint heading;
					heading = center.add(DPoint.newPolar(radius, curShipIndex * fighterAngleStep));
					cur.setHeading(shortestAngleToPoint(cur.getPosition(), heading));
				} else {
					double distance = distance(cur.getPosition(), target.getPosition());
					
					cur.setHeading(shortestAngleToPoint(cur.getPosition(), target.getPosition()));
		...
		myShips.apply(AIFilter.MISSILES, new AIGovernor() {
			...
			{
				if (target != null) {
					s.setHeading(shortestAngleToPoint(s.getPosition(), target.getPosition()));
				}
			}
		});
		...
		if (target == null) {
			...
			myCruiser.setScannerHeading(shortestAngleToPoint(myCruiser.getPosition(), target.getPosition()));
	...
	private double shortestAngleToPoint(DPoint from, DPoint to)
	{
		double x1 = from.getX();
		double y1 = from.getY();
		double x2 = to.getX();
