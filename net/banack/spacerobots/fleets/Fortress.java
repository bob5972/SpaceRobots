...
public class Fortress extends AIFleet
{
	...
	public Iterator<ShipAction> runTick()
	{
		...
		if (shipCreateCount % 3 == 2) {
			if (myCruiser.canLaunch(CRUISER) && credits >= CRUISER.getCost() + myShips.size() * 3 * MISSILE.getCost()) {
		...
		while (i.hasNext()) {
			...
			if (target != null) {
				...
				if (distance < 60 && cur.canLaunch(MISSILE) && !launchedMissleThisTick) {
					cur.launch(MISSILE);
			...
			if (myCruiser.isAlive()) {
				cur.setHeading(shortestAngleToPoint(cur.getPosition(), myCruiser.getPosition().add(
				        DPoint.newPolar(CRUISER.getScannerRadius() / 3, shipCount * Math.PI * 2 / 5))));
			} else {
				cur.setHeading(shortestAngleToPoint(cur.getPosition(), destination));
			}
		}
