 ...
 * <p>
 * It also contains a set of utility functions for text processing.
 */
public class SpaceText extends Parser
{
	...
	{
		return f.getFleetID() + " " + f.getTeamID() + " " + " \"" + f.getFleetName() + "\" \"" + f.getAIName()
		        + "\" \"" + f.getAIAuthor() + "\" \"" + f.getAIVersion() + "\" " + (f.isAlive() ? 1 : 0);
	}
	...
	public static ShipAction parseAction(String[] inp)
	{
		...
		if (inp.length < 5)
			throw new IllegalArgumentException("Invalid ShipAction String: Expected 6 parameters, received "
			        + inp.length);
		int x = 0;
