...
public class SpaceRobotsUI {
	...
	runBattleButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			startingCredits =
			    Integer.parseInt(startingCreditsField.getText());
			creditsPerTurn =
			    Integer.parseInt(creditsPerTurnField.getText());
		    ...
		    new Thread(new Runnable() {
			    public void run() {
				runBattle(width, height, ai, startingCredits,
					  creditsPerTurn, r.nextLong(),
					  new GLDisplay());
			    }
			}).start();
		}
    ...
    private static void addAIFleets(Battle b,
				    AIFleet[] ai,
				    int startingCredits,
				    int creditIncrement) {
	...
	try {
		...
		ServerAIProtocol aip =
		    ServerProtocolFactory.doHandshake(sIn,sOut);
		b.addFleet("Fleet "+x, new FleetAI(aip), tID[x],
			   startingCredits, creditIncrement);
	    }
	} catch(java.io.IOException e) {
